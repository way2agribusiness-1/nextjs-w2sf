...
export const getCrops =
  ...
  async (dispatch) => {
    try {
      ...
      let url = `/api/v1/israel-agritech?keyword=${keyword}&price[gte]=${price[0]}&price[lte]=${price[1]}&ratings[gte]=${ratings}&page=${currentPage}`;
      if (category) {
        url = `/api/v1/israel-agritech?keyword=${keyword}&category=${category}&price[gte]=${price[0]}&price[lte]=${price[1]}&ratings[gte]=${ratings}&page=${currentPage}`;
      }
...
export const getAdminCrops = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get("/api/v1/admin/israel-agritech");
...
export const getCropDetails = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`/api/v1/israel-agritech/${id}`);
...
export const deleteCrop = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(`/api/v1/admin/israel-agritech/${id}`);
