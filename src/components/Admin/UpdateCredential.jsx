...
function Credentials() {
  ...
  async function fetchData() {
    const { data } = await axios.get(`/api/v1/admin/credential/${id}`);
    const { name, image, alt, title, type } = data.credential;
    setName(name);
    setAlt(alt);
    setTitle(title);
    setType(type);
  ...
  async function handleSubmit() {
    ...
    if (type === 'select') {
      enqueueSnackbar('Select Type of Image', { variant: 'warning' });
      return;
    }
    if (!image) {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      >
        ...
        <MenuItem value={'Media Coverages'}>Media Coverages</MenuItem>
        <MenuItem value={'Approvals and Licenses'}>
